...
extension MovieVO {
    ...
    static func getMoviesByBookMarkFetchRequest(bookmark : BookMarkVO) -> NSFetchRequest<BookMarkVO> {
        let fetchRequest : NSFetchRequest<BookMarkVO> = BookMarkVO.fetchRequest()
        let sortDescriptor = NSSortDescriptor(key: "movieId", ascending: false)
        fetchRequest.sortDescriptors = [sortDescriptor]

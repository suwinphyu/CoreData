...
class MovieDetailsViewController: UIViewController {
    ...
    fileprivate func initBookMarkView(){
        isBookMarkSelected = BookMarkVO.isSelectBookMark(movieId: movieId)
        bookMarkButton.tintColor = isBookMarkSelected ? #colorLiteral(red: 1, green: 0.3098039216, blue: 0.2666666667, alpha: 1) : #colorLiteral(red: 1.0, green: 1.0, blue: 1.0, alpha: 1.0)
    }
    
    @IBAction func clickBookMark(_ sender: Any) {
        if isBookMarkSelected == true {
            isBookMarkSelected = false
            bookMarkButton.tintColor = #colorLiteral(red: 1.0, green: 1.0, blue: 1.0, alpha: 1.0)
            BookMarkVO.deleteBookMarkVO(movieId: movieId, context: CoreDataStack.shared.viewContext)
          
        } else {
            isBookMarkSelected = true
            bookMarkButton.tintColor = #colorLiteral(red: 1, green: 0.3098039216, blue: 0.2666666667, alpha: 1)
            BookMarkVO.saveBookMarkVO(movieId: movieId, context: CoreDataStack.shared.viewContext)
        }
    }
